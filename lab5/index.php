<!DOCTYPE html>
<html>
<head>
<!--
Craig Mazzotta
cs 0334
Lab 5
-->
<meta charset="utf-8">
<title>Lab 5-Craig Mazzotta</title>
<link rel="stylesheet" href="main.css" media="screen" title="no title">
</head>
<body>
<h1>Dice Roll Game</h1>

<?php
global $rollSum;

printGreeting();
printform();

function printGreeting(){
global $rollSum;
$guess = filter_input(INPUT_POST, "guess");
$numDice = filter_input(INPUT_POST, "numDice");

if(!filter_has_var(INPUT_POST,"guess")){
echo "<h2>Welcome to My Dice Rolling Game</h2>";
}
else if($guess == $rollSum) {
rollDice($numDice);
echo "<h3>Good Guess, You Win!</h3>";
}
else {
  rollDice($numDice);

print <<<HERE
<h2>Wrong! Try Again!</h2>
<p>
Your guess: $guess <br>
Actual total: $rollSum
</p>
HERE;
}
}

function rollDice($numDice){
  global $rollSum;
  echo "<h2>New Roll:</h2><br>";
  $rollSum = 0;
  $dice = 0;

  for($i=0; $i<$numDice; $i++){
    $dice = rand(1,6);
    printDice($dice);
    $rollSum += $dice;
  }
  return $rollSum;
}

function printDice($val){
print <<<HERE
<img src="dice/die$val.jpg" alt="die:$val"/>
HERE;
}


function printForm() { //where people will submit their choices and guesses
global $total;

print <<<HERE
<form method="post" action="">
<fieldset>
<label> Choose the number of dice you would like to roll:</label>
<select name="numDice">
<option value="1">1</option>
<option value="2">2</option>
<option value="3">3</option>
<option value="4">4</option>
<option value="5">5</option>
</select>
</fieldset>
<br/>
<label>What do you think the number will be?:</label>
<select name="guess">
<option value="1">1</option>
<option value="2">2</option>
<option value="3">3</option>
<option value="4">4</option>
<option value="5">5</option>
<option value="6">6</option>
<option value="7">7</option>
<option value="8">8</option>
<option value="9">9</option>
<option value="10">10</option>
<option value="11">11</option>
<option value="12">12</option>
<option value="13">13</option>
<option value="14">14</option>
<option value="15">15</option>
<option value="16">16</option>
<option value="17">17</option>
<option value="18">18</option>
<option value="19">19</option>
<option value="20">20</option>
<option value="21">21</option>
<option value="22">22</option>
<option value="23">23</option>
<option value="24">24</option>
<option value="25">25</option>
<option value="26">26</option>
<option value="27">27</option>
<option value="28">28</option>
<option value="29">29</option>
<option value="30">30</option>
</select>
<br/>
<input type="submit" />
</fieldset>
</form>
HERE;
} //end printForm

        ?>
     <p><a href="php.txt">php code</a></p>
     <p><a href="css.txt">css code</a></p>
  </body>
</html>
